...
export const web3Login = (walletData) => async (dispatch) => {
    try {
        dispatch(start())
        const { data } = await api.web3Login(walletData)
        ...
        const message = err?.response?.data?.message || err?.message || "Web3 login failed"
        toast.error(message)
        dispatch(error(err.message))
        throw err
    }
